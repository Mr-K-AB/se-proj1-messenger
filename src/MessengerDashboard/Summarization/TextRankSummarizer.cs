...
namespace MessengerDashboard.Summarization
{
    ...
    public class TextRankSummarizer : ITextSummarizer
    {
        public TextSummary Summarize(string[] sentences, TextSummarizationOptions options)
        {
            ...
            TextSummary textSummary = new(rankedSentences);
            return textSummary;
        }
        ...
        double CosineSimilarity(string sentence1, string sentence2)
        {
        ...
        double[,] CalculateSimilarityMatrix(List<string> sentences)
        {
    ...
    double[] CalculateTextRank(double[,] similarityMatrix, double damping = 0.85, double tolerance = 0.0001, int maxSteps = 100)
    {
